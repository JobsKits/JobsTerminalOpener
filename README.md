...
`JobsTerminalOpener` 是一个 [**Swift**](https://www.swift.org/) macOS App + Finder Sync Extension 工程，用来把“用终端打开”、iOS 依赖安装、[**Flutter**](https://flutter.dev/) 依赖拉取和 [**CodeGraph**](https://github.com/colbymchenry/codegraph) 代码地图安装放到 Finder 右键一级菜单区域。
...
4、回到 Finder，右键任意文件或文件夹，点击 `用终端打开`；右键合法的 CocoaPods iOS 工程或 Flutter 工程根目录时，会额外显示对应的依赖操作；右键普通文件夹时，可以安装或升级 CodeGraph 并为当前文件夹建立代码地图。
...
- Finder 扩展和宿主 App 共用同一套项目合法性校验，菜单展示前校验一次，真正执行前再次校验，避免选择后目录内容变化或外部 URL Scheme 绕过条件。
